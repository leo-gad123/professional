    ...
    <section id="experience" className="relative px-4 sm:px-6 py-16 md:py-24">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="relative pl-10 md:pl-14">
          ...
          {roles.map((r, i) => (
            <div key={r.title} className="relative mb-10 last:mb-0 animate-fade-in-up" style={{ animationDelay: `${0.1 + i * 0.2}s` }}>
              ...
              <div className="bg-card rounded-2xl p-7 card-shadow">
                ...
                <h3 className="mt-2 font-serif text-2xl md:text-3xl font-medium text-foreground">{r.title}</h3>
                <ul className="mt-4 grid gap-2.5 sm:grid-cols-3 stagger-children">
                  {r.items.map((it) => (
                    <li key={it} className="text-sm text-muted-foreground flex items-start gap-2.5 bg-secondary/50 rounded-xl px-4 py-3">
